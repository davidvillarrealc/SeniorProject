...
masterTableCopy$repSE <- apply(masterTableCopy, 1, function(x){predict.lm(simpleModelRepublican, data.frame(GrowthRate = as.numeric(x["GrowthRate"]), I = as.numeric(x["I"]), Inflation = as.numeric(x["Inflation"]), GoodGrowthQuarters = as.numeric(x["GoodGrowthQuarters"]), DUR2 = as.numeric(x["DUR2"]), repLaggedVoteShare = as.numeric(x["repLaggedVoteShare"])), se.fit = TRUE)[[2]][1]})
                                 
 


masterTableCopy$MCSimulation <- apply(masterTableCopy, 1, function(x){rnorm(1, as.numeric(x["estimate"]), as.numeric(x["SE"]))})
masterTableCopy$MCSimulationREP <- apply(masterTableCopy, 1, function(x){rnorm(1, as.numeric(x["repEstimate"]), as.numeric(x["repSE"]))})


#Add prediction Democratic wins
 masterTableCopy$MCWIN <- ifelse(masterTableCopy$MCSimulation >= masterTableCopy$MCSimulationREP, 1, 0)
 sum(subset(masterTableCopy, year == 2012)$MCWIN)
...
sum(subset(masterTableCopy, year == 2000)$elecVotesWon)
